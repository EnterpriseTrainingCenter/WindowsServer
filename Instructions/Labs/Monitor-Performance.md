...
On **VN1-SRV2**, sign in as **ad\Administrator**.
...
1. [Configure a data collector set with baseline performance counters](#task-1-configure-a-data-collector-set-with-baseline-performance-counters) on VN1-SRV2
1. [Configure VN1-SRV2 with 1 GB of memory](#task-2-configure-VN1-SRV2-with-1-gb-of-memory)
1. [Start the data collector set](#task-3-start-the-data-collector-set
1. [Simulate load](#task-4-simulate-load) by copying an ISO file from C:\\LabResources of VN1-SRV2 to CL1.
...
### Task 2: Configure VN1-SRV2 with 1 GB of memory
...
Wait for the complete start of VN1-SRV2.
...
1. Measure the time it takes to copy **\\\\VN1-SRV2\\c$\\LabResources\\20348.1.210507-1500.fe_release_amd64fre_SERVER_LOF_PACKAGES_OEM.iso** to **C:\\**.
   ...
   Measure-Command { 
      Copy-Item `
         -Path `
            '\\VN1-SRV2\c$\LabResources\20348.1.210507-1500.fe_release_amd64fre_SERVER_LOF_PACKAGES_OEM.iso' `
         -Destination 'c:\' `
          -Force 
      }
   Get-Date
...
1. Click **Add...**
1. In Select log file, in **File name**, enter **\\\\VN1-SRV2\\c$\\PerfLogs\\Admin\\Baseline**.
...
1. [Configure VN1-SRV2 with 4 GB of memory](#task-1-configure-VN1-SRV2-with-4-gb-of-memory)
...
### Task 1: Configure VN1-SRV2 with 4 GB of memory
