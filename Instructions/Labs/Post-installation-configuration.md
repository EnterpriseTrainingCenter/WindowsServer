...
1. [Set the default Administrator password](#task-1-set-the-default-administrator-password-on-VN1-SRV10)
1. [Configure the Ethernet adapter with the IPv4 address 10.1.1.56/24, default gateway 10.1.1.1 and DNS server 10.1.1.8](#task-2-configure-the-ethernet-adapter-on-VN1-SRV10)
1. [Set the computer name to VN1-SRV10 and join the domain ad.adatum.com](#task-3-set-the-computer-name-to-VN1-SRV10-and-join-the-domain)
1. [Add the VN1-SRV10 to Windows Admin Center](#task-4-add-the-VN1-SRV10-to-windows-admin-center)

    > Why does single sign-on not work with the new server?

1. [Set the time zone](#task-5-set-the-time-zone-on-VN1-SRV10)

### Task 1: Set the default Administrator password on VN1-SRV10
...
### Task 2: Configure the Ethernet adapter on VN1-SRV10
...
1. In **IP address**, enter 10.1.1.56.
...
1. Configure the network adapter **Ethernet** with the IP address **10.1.1.56/24** and the default gateway 10.1.1.1.
...
### Task 3: Set the computer name to VN1-SRV10 and join the domain
...
1. In Computer Name/Domain Change, in **Computer name**, enter **VN1-SRV10**.
...
1. On the tab **Add one**, type 10.1.1.56.
...
1. Click **10.1.1.56**.
1. Connected to 10.1.1.56, in Overview, click **Edit Computer ID**.
1. In the pane Edit computer ID, in **Computer name**, enter **VN1-SRV10**.
1. Click **Domain** and enter **ad.adatum.com.**.
1. Click **Next**.
...
1. In Windows Admin Center, activate the checkbox beside **10.1.1.56**.
1. Click **Remove**.
...
1. Run **Windows PowerShell** as Administrator.
1. Rename the computer to **VN1-SRV10** and join it to the domain **ad.adatum.com**.

    ````powershell
    Add-Computer -NewName VN1-SRV10 -DomainName ad.adatum.com -Restart
    ````

1. Enter the credentials for **ad\Administrator**.

### Task 4: Add the VN1-SRV10 to Windows Admin Center
...
1. Enter **VN1-SRV10** and click **Search**.
1. Activate the checkbox left to **VN1-SRV10.ad.adatum.com** and click **Add**.
...
1. Under Active Directory Domain Services, in **Search Active Directory**, type **VN1-SRV10** and click **Search**.
1. In the results, click **VN1-SRV10** and click **Properties+*.
1. In Computer properties: VN1-SRV10$, click **Add a Windows Admin Center gateway**.
...
### Task 5: Set the time zone on VN1-SRV10

#### Desktop Experience

Perform this task von VN1-SRV10.

1. Logon as **ad\Administrator**.
1. In **Server Manager**, in the left pane, click **Local Server**.
1. In Properties For VN1-SRV10, right to **Time zone**, click **(UTC-08:00) Pacific Time (US & Canada)**.
...
1. Open **Terminal**.
1. Set the time zone of VN1-SRV10 to the same time zone as CL1.

    ````powershell
    $timezone = Get-TimeZone
    Invoke-Command -ComputerName VN1-SRV10 { Set-TimeZone $using:timezone.id }
    ````

## Exercise 2: Configure password, TCP/IP, domain join and time zone on Windows Server

1. [Set the default Administrator password](#task-1-set-the-default-administrator-password-on-VN1-SRV11)
1. [Configure the Ethernet adapter with the IPv4 address 10.1.1.56/24, default gateway 10.1.1.1 and DNS server 10.1.1.8](#task-2-configure-the-ethernet-adapter-on-VN1-SRV11)
1. [Set the computer name to VN1-SRV10 and join the domain ad.adatum.com](#task-3-set-the-computer-name-to-VN1-SRV11-and-join-the-domain)
1. [Add the VN1-SRV10 to Windows Admin Center](#task-4-add-the-VN1-SRV11-to-windows-admin-center)

    > Why does single sign-on not work with the new server?

1. [Set the time zone](#task-5-set-the-time-zone-on-VN1-SRV11)

### Task 1: Set the default Administrator password on VN1-SRV11

Perform this task on VN1-SRV11.
...
### Task 2: Configure the Ethernet adapter on VN1-SRV11
...
### Task 3: Set the computer name to VN1-SRV11 and join the domain
...
1. Beside **Enter new computer name (Blank=Cancel)**, enter **VN1-SRV11**.
1. Enter the password for ad\Administrator.
...
1. In the pane Edit computer ID, in **Computer name**, enter **VN1-SRV11**.
1. Click **Domain** and enter **ad.adatum.com.**.
1. Click **Next**.
...
1. Rename the computer to **VN1-SRV11** and join it to the domain **ad.adatum.com**.

    ````powershell
    Add-Computer -NewName VN1-SRV11 -DomainName ad.adatum.com -Restart
    ````

1. Enter the credentials for **ad\Administrator**.

### Task 4: Add the VN1-SRV11 to Windows Admin Center

Perform this task von CL1.

1. Open **Terminal**.
1. Configure Kerberos constrained delegation to allow the computer account of **VN1-SRV10** to be delegated to the Windows Admin Center gateway **VN1-SRV4**.

    ````powershell
    $gateway = Get-ADComputer VN1-SRV4
    Set-ADComputer VN1-SRV11 -PrincipalsAllowedToDelegateToAccount $gateway
    ````
...
1. In Add or create resources, under **Servers**, click **Add**.
1. Click the tab **Search Active Directory**.
1. Enter **VN1-SRV11** and click **Search**.
1. Activate the checkbox left to **VN1-SRV11.ad.adatum.com** and click **Add**.
1. In Windows Admin Center, click **VN1-SRV11.ad.adatum.com.**.
...
### Task 5: Set the time zone on VN1-SRV11

#### SConfig

Perform this task von VN1-SRV11.
...
1. Set the time zone of VN1-SRV10 to the same time zone as CL1.
    ...
    ````powershell
    $timezone = Get-TimeZone
    Invoke-Command -ComputerName VN1-SRV11 { Set-TimeZone $using:timezone.id }
    ````
