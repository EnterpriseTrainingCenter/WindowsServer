...
Create the shares on VN1-FS1 and configure the permissions according to the table above. Leave the permissions for Administrators, CREATOR OWNER and SYSTEM. Use different administation tools for this task. Below, generic instructions for each tool are provided.
...
1. Under **Permission entries**, click the first entry with the **Principal** **Users (VN1-FS1\\Users)** and click **Remove**. Repeat this step for all entries except for those with Principal **Administrators**, **SYSTEM**, and **CREATOR OWNER**.
1. Click **Add**.
...
1. In File Explorer, copy the contents of the respective folder in \\\\VN1-FS1\\c$\LabResources\\Sample Documents to the share with the same name on VN1-FS1.
...
1. On VN1-FS1, copy the contents of the respective folder in **C:\\LabResources\\Sample Documents** to **D:\\Shares\\...**.
...
1. Using **File Explorer**, navigate to **\\\\VN1-FS1**.
...
1. [Verify offline access](#task-4-verify-offline-access).
    ...
    > Which folders and files are available in  \\\\VN1-FS1\\IT?

    > Which folders and files are available in  \\\\VN1-FS1\\Finance?

    > Which folders and files are available in  \\\\VN1-FS1\\Marketing?

1. [Bring CL2 online](#task-5-bring-computer-online).
1. [Verify online access](#task-6-verify-online-access).

    > Can you open \\\\VN1-FS1\\Marketing\\Building Partnerships.pptx?

### Task 1: Configure shares for offline access

Configure the Offline Settings of the shares on VN1-FS1 according to the table above. Use different administation tools for this task. Below, generic instructions for each tool are provided.
...
1. In Select Computer, in **Another computer**, type **VN1-FS1** and click **OK**.
1. Expand **Computer Management (VN1-FS1)**, **System Tools**, **Shared Folders**, and click **Shares**.
...
1. Navigate to \\\\VN1-FS1\\Marketing.
...
1. Using **File Explorer**, navigate to \\\\VN1-FS1\\Marketing.
...
> NotD: The configuration interface for Volume Shadow Copies is available on NTFS volumes only, although ReFS supports Volume Shadow Copies.
...
1. In Finance (\\\\vn1-fs1) Properties, click the tab **Previous Versions**.
